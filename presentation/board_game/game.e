		...
		do
			...
			invariant
				...
				across 1 |..| (i-1) as k all
					...
					players.sequence[k.item].position = {BOARD}.Start_position and
			...
			not_yet_played: winners.sequence.is_empty
			fair_start: across players.sequence as o all o.item.position = {BOARD}.Start_position end
...
feature -- Access
	...
	winners: V_LIST [PLAYER]
			-- Winners (Empty if the game if not over yet).
